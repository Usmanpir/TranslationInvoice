...
import { Edit2, Trash2, CheckCircle, XCircle, ArrowLeft, FileCheck, FileQuestion, FileX2 } from 'lucide-react'
  ...
  if (!invoice) return (
    <div className="p-4 sm:p-6 lg:p-10">
      <EmptyState
        ...
        action={<Link href="/invoices" className="btn-secondary"><ArrowLeft className="w-4 h-4" />Back to invoices</Link>}
      />
    </div>
  )
    ...
    <div>
      ...
      <div className="p-4 sm:p-6 lg:p-10">
        <div className="max-w-4xl mx-auto xl:mx-0">
          {invoice.quotation && (
            <div className="mb-5 flex items-center gap-2.5 px-4 py-3 rounded-xl bg-gradient-to-r from-purple-50 to-violet-50/40 border border-purple-100 text-sm text-purple-800 animate-fade-in">
          ...
          <div className="relative card overflow-hidden p-5 sm:p-10 shadow-elevated print:shadow-none animate-fade-up" id="invoice-print">
            <div aria-hidden className="absolute inset-x-0 top-0 h-1.5 bg-gradient-to-r from-brand-500 via-indigo-500 to-violet-500" />

            {/* Company Logo / Name centered */}
            <div className="text-center mb-6">
              <div className="inline-flex items-center gap-2 mb-1">
                <LogoMark className="w-10 h-10" />
                <h2 className="font-display text-xl font-bold text-slate-900">{user?.companyName || user?.name}</h2>
              </div>
            </div>

            {/* Title */}
            <div className="text-center mb-8">
              <h1 className="font-display text-3xl font-extrabold tracking-tight text-slate-900 border-b-2 border-slate-200 pb-3 inline-block px-8">
                Invoice
              </h1>
              <div className="mt-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-8">
              ...
              <div className="rounded-xl bg-slate-50/70 border border-slate-200/70 p-5">
                <p className="text-[11px] font-bold text-brand-600 uppercase tracking-[0.1em] mb-2">From:</p>
              ...
              {/* To */}
              <div className="rounded-xl bg-slate-50/70 border border-slate-200/70 p-5">
                <p className="text-[11px] font-bold text-brand-600 uppercase tracking-[0.1em] mb-2">To:</p>
            ...
            <div className="rounded-xl border border-slate-200/80 p-4 sm:p-5 mb-6">
              <div className="flex items-center justify-between flex-wrap gap-4">
                <div>
                  <p className="font-display text-lg font-bold text-slate-900">Invoice # <span className="font-mono">{invoice.invoiceNumber}</span></p>
                </div>
                ...
                {invoice.paymentProof.match(/\.(jpg|jpeg|png|webp|gif)$/i) ? (
                  <a href={invoice.paymentProof} target="_blank" rel="noopener noreferrer">
                    <img
                      ...
                      alt="Payment proof"
                      className="max-h-60 rounded-xl border border-slate-200 object-contain shadow-card hover:shadow-card-hover transition-shadow"
                    />
                  </a>
                ) : (
                  <a
                    ...
                    className="inline-flex items-center gap-2 px-4 py-2.5 bg-brand-50/60 border border-brand-100 rounded-xl text-sm font-medium text-brand-700 hover:bg-brand-50 transition-colors"
                  >
