...
function NavLink({ item, active, onClick }: { item: NavItem; active: boolean; onClick?: () => void }) {
  return (
    ...
    >
      ...
      <item.icon
        className={cn('w-[18px] h-[18px] flex-shrink-0 transition-colors', active ? 'text-brand-300' : 'text-slate-500 group-hover:text-slate-300')}
      />
...
function WorkspaceSwitcher() {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-slate-700 to-slate-800 ring-1 ring-white/10 flex items-center justify-center text-xs font-bold text-white flex-shrink-0 overflow-hidden">
          {organization.logoUrl ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={organization.logoUrl} alt="" className="w-full h-full object-contain bg-white" />
          ) : (
            organization.name[0]?.toUpperCase()
          )}
        </div>
        <div className="min-w-0 flex-1">
          <p className="text-[13px] font-semibold text-slate-100 truncate">{organization.name}</p>
      ...
      {open && (
        <div className="absolute left-0 right-0 top-full mt-2 z-50 rounded-xl bg-slate-900 ring-1 ring-white/10 shadow-2xl p-1.5 animate-scale-in">
          <p className="px-2.5 pt-1.5 pb-1 text-[10px] font-semibold text-slate-500 uppercase tracking-[0.12em]">Workspaces</p>
          {!options ? (
            <div className="px-2.5 py-2 text-xs text-slate-500">Loading…</div>
          ) : (
            options.map((o) => (
              <button
                key={o.id}
                onClick={() => switchTo(o.id)}
                className="w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-sm text-slate-300 hover:bg-white/[0.06] hover:text-white"
              >
                <span className="flex-1 truncate text-left">{o.name}</span>
                <span className="text-[10px] text-slate-500 capitalize">{o.role.toLowerCase()}</span>
          ...
          <Link
            ...
            className="mt-1 flex items-center gap-2 px-2.5 py-2 rounded-lg text-sm text-slate-400 hover:bg-white/[0.06] hover:text-white border-t border-white/5"
          >
      ...
      <nav className="relative flex-1 px-3 py-2 space-y-0.5 overflow-y-auto">
        {sections.map((section, i) => (
          <div key={section.title ?? i} className={cn(i > 0 && 'pt-4')}>
            {section.title && (
              <p className="px-3 pb-1.5 text-[10px] font-semibold text-slate-500 uppercase tracking-[0.12em]">{section.title}</p>
            )}
        ...
        {user.isSuperAdmin && (
          <div className="pt-4">
            <p className="px-3 pb-1.5 text-[10px] font-semibold text-slate-500 uppercase tracking-[0.12em]">Platform</p>
      ...
      {mounted &&
        ...
        createPortal(
          <div className="lg:hidden fixed inset-0 z-[60]">
            <div className="absolute inset-0 bg-slate-950/60 backdrop-blur-sm animate-fade-in" onClick={() => setOpen(false)} aria-hidden />
            <div className="absolute inset-y-0 left-0 w-72 max-w-[85vw] bg-slate-950 shadow-2xl flex flex-col animate-slide-in-left">
              <button
                type="button"
                onClick={() => setOpen(false)}
                aria-label="Close menu"
                className="absolute top-5 right-3 z-10 w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-white hover:bg-white/10"
              >
                <X className="w-4 h-4" />
              </button>
